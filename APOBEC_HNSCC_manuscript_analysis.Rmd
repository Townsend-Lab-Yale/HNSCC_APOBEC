...
signature.breakdown.means.HPVneg <- as.data.frame(matrix(data=NA, nrow=length(unique(signature.breakdown.HPVneg$Name)),ncol=3))
colnames(signature.breakdown.means.HPVneg) <- c("Name","median APOBEC","median NonAPOBEC")
...
signature.breakdown.means.HPVpos <- as.data.frame(matrix(data=NA, nrow=length(unique(signature.breakdown.HPVpos$Name)),ncol=3))
colnames(signature.breakdown.means.HPVpos) <- c("Name","median APOBEC","median NonAPOBEC")
...
HPVneg.prev.df.merged <- as.data.frame(matrix(ncol=5,nrow=length(unique(HPVneg.prev.df$short_name)),data=NA))
colnames(HPVneg.prev.df.merged) <- c("Short name", "Median APOBEC contribution", "Median NonAPOBEC contribution", "Frequency", "Selection intensity")
HPVneg.prev.df.merged$`Short name` <- unique(HPVneg.prev.df$short_name)

HPVpos.prev.df.merged <- as.data.frame(matrix(ncol=5,nrow=length(unique(HPVpos.prev.df$short_name)),data=NA))
colnames(HPVpos.prev.df.merged) <- c("Short name", "Median APOBEC contribution", "Median NonAPOBEC contribution", "Frequency", "Selection intensity")
