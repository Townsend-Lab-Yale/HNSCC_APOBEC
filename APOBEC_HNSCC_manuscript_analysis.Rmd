...
ggsave(paste("Figures/","HPVpos","_trinuc_heatmap.png",sep=""),height = 2.5,width = 10,plot = HPV.pos.trinuc.heatmap)
...
ggsave(filename = paste("Figures/","HPVneg","_trinuc_heatmap.png",sep=""),height = 2.5,width = 10,plot = HPV.neg.trinuc.heatmap)
...
mutation_rates_full_scatter <- ggplot(data = mutation_rates, aes(x=positive_mut_rates,y=negative_mut_rates)) + geom_point(alpha=0.2,col="black",size=0.5) + geom_text_repel(data=subset(mutation_rates, positive_mut_rates > 5e-5 | negative_mut_rates > 3e-5 ), aes(label=gene),size=3) + geom_point(data = subset(mutation_rates, gene %in% c("FBXW7","PIK3CA","MAPK1")),aes(x=positive_mut_rates,y=negative_mut_rates),size=3,col="blue") + geom_text_repel(data = subset(mutation_rates, gene %in% c("FBXW7","PIK3CA","MAPK1","TP53")),aes(x=positive_mut_rates,y=negative_mut_rates,label=gene),size=5,col="blue",fontface="bold") +
  labs(x="Mutation rate in HPV+ tumors",y="Mutation rate in HPV- tumors") + coord_equal(ratio=1) + theme_bw()+ geom_smooth(method='lm',formula=y~x,color="red") + geom_abline(slope=1, intercept=0) + scale_x_continuous(labels=fancy_scientific) + scale_y_continuous(labels=fancy_scientific)
...
summary(lm(data = mutation_rates,formula = negative_mut_rates~positive_mut_rates))

ggsave(plot = mutation_rates_full_scatter,filename = "Figures/mutation_rates_full_scatter.png")

ggsave(plot = mutation_rates_reduced,filename = "Figures/mutation_rates_reduced.png")
...
for(i in 1:nrow(segment_data_tumors[which(segment_data_tumors$xend==0),])){
  segment_data_tumors$HPV_status[which(segment_data_tumors$xend==0)][i] <- apobec_hnscc_df$categ[which(apobec_hnscc_df$patient_id==segment_data_tumors$tumor[which(segment_data_tumors$xend==0)][i])]
}
...
plt_hmap <- ggplot(heatmap_data, 
  ...
  labs(x = "Signature", y = "") +
  theme_bw() +
  theme(axis.text.x.top = element_text(size = rel(1), vjust=0.5,hjust=0.5 ,  angle = 90),axis.title.y = element_blank(),
        # margin: top, right, bottom, and left
        plot.margin = unit(c(-.051, -.051, -.051, -.051), "cm"),
...
ggsave(filename = "Figures/heatmap_and_dendro.png",plot = heatmap_and_dendro,height = 9,width = 6)
...
# library(ggplot2)
ggsave(plot = HPV_status_vs_APOBEC_weight, filename ="Figures/HPV_status_APOBEC_weight.png",width = 5,height = 3)
...
selection.from.contexts.labels <- ggplot(data = ALL.prev.df.split) +#geom_violin(aes(x=`APOBEC context`, y = `Selection from mutational context`))  +
  ...
  labs(y="Net realized selection intensity") + theme_bw() + theme(axis.text=element_text(size=16), axis.title=element_text(size=18,face="bold"))
...
for(i in 1:nrow(signature.breakdown.ALL)){
  if(length(which(apobec_hnscc_df$patient_id==signature.breakdown.ALL$Tumor[i]))==1){
    signature.breakdown.ALL$HPV_status[i] <- apobec_hnscc_df$categ[which(apobec_hnscc_df$patient_id==signature.breakdown.ALL$Tumor[i])]
  }
