...
prev_plot <- ggplot(data = prevalence.df) + 
  ...
  geom_text_repel(data = subset(prevalence.df, (prev_HPVneg>0.025 | prev_HPVpos>0.04) | (prev_HPVneg>0.0 & prev_HPVpos>0)),aes(y = prev_HPVneg, x = prev_HPVpos,label=Name),box.padding =0.6,size=common.text.size*(5/14),color="black") + 
  theme_classic() + 
...
library(cowplot)
heatmap_and_dendro <- plot_grid(NULL,plt_dendr_signature,plt_dendr, plt_hmap, align = 'hv', rel_widths = c(0.6, 1), rel_heights = c(.4,2))
...
dendro_and_heatmap_labs <- ggdraw() + 
  ...
  geom_text(x=.03,y=.42,label="Tumors",size=6,angle=90)
