...
```{r loading and splitting MAF file, message=TRUE, warning=FALSE}
...
# Saved sheet 10 of the supp. data from Hedberg et al. J Clin Invest. 2016;126(1):169-180. doi:10.1172/JCI82066 as a CSV. 
# Hedberg.2016 <- read.csv(file = "input_data/Hedberg_2016/JCI82066.sdt1-10_sheet10.csv",header = T,stringsAsFactors = F,skip = 1)
# 
# unique(Hedberg.2016$Variant.Status)
# 
# # Just want variants found in primary tumors 
# Hedberg.2016 <- Hedberg.2016[which(Hedberg.2016$Variant.Status %in% c("Transmitted","Primary Only")),]
# 
# # changing column names to align with those from TCGA 
# colnames(Hedberg.2016)[which(colnames(Hedberg.2016)=="Gene")] <- "Hugo_Symbol"
# colnames(Hedberg.2016)[which(colnames(Hedberg.2016)=="Chr")] <- "Chromosome"
# colnames(Hedberg.2016)[which(colnames(Hedberg.2016)=="Position")] <- "Start_Position"
# Hedberg.2016$End_Position <- Hedberg.2016$Start_Position
# Hedberg.2016$Tumor_Sample_Barcode <- Hedberg.2016$Patient
# Hedberg.2016$Variant_Type <- "SNP"
# 
# Hedberg.2016$Chromosome[which(Hedberg.2016$Chromosome=="chr24")] <- "chrX"
# 
# # delete Hedberg 2016 variants without a clear call
# if(length(which(nchar(Hedberg.2016$Base.Change)>3))>0){
# Hedberg.2016 <- Hedberg.2016[which(nchar(Hedberg.2016$Base.Change)==3),]
# }
# 
# Hedberg.2016$Reference_Allele <- NA
# Hedberg.2016$Tumor_Seq_Allele1 <- NA
# Hedberg.2016$Tumor_Seq_Allele2 <- NA
# 
# split.chromosome <- strsplit(x=Hedberg.2016$Chromosome,split = "chr")
# split.bases <- strsplit(x = Hedberg.2016$Base.Change,split = ">")
# for(i in 1:nrow(Hedberg.2016)){
#  Hedberg.2016[i,c("Reference_Allele","Tumor_Seq_Allele1")] <-  split.bases[[i]][1]
#  Hedberg.2016[i,"Tumor_Seq_Allele2"] <- split.bases[[i]][2]
#  Hedberg.2016[i,"Chromosome"] <- split.chromosome[[i]][2]
# }
# 
# to.add <- as.data.frame(matrix(data = NA,nrow = nrow(Hedberg.2016),ncol = ncol(HNSC.MAF)))
# colnames(to.add) <- colnames(HNSC.MAF)
# HNSC.MAF <- rbind(HNSC.MAF,to.add)
# 
# HNSC.MAF[which(is.na(HNSC.MAF$Hugo_Symbol)),c("Hugo_Symbol","Chromosome","Start_Position","End_Position","Tumor_Sample_Barcode","Variant_Type","Reference_Allele","Tumor_Seq_Allele1","Tumor_Seq_Allele2")] <- Hedberg.2016[,c("Hugo_Symbol","Chromosome","Start_Position","End_Position","Tumor_Sample_Barcode","Variant_Type","Reference_Allele","Tumor_Seq_Allele1","Tumor_Seq_Allele2")]


Hedberg.2016 <- read.csv(file = "output_data/yale_filtered.maf",header = T,sep = "\t",stringsAsFactors = F)
as.character(unique(Hedberg.2016$Tumor_Sample_Barcode))
...
length(which((trinuc.w.HPV$`Signature.2` > 0 | trinuc.w.HPV$`Signature.13`>0) & trinuc.w.HPV$HPV=="HPV−"))/length(which(trinuc.w.HPV$HPV=="HPV−"))
...
prev_plot <- ggplot(data = prevalence.df) + 
  ...
  geom_text_repel(data = subset(prevalence.df, (prev_HPVneg>0.025 | prev_HPVpos>0.04) | (prev_HPVneg>0.0 & prev_HPVpos>0)),aes(y = prev_HPVneg, x = prev_HPVpos,label=Name),box.padding =0.7,size=common.text.size*(5/14),color="black") + 
  theme_classic() + 
...
mutation_rates_full_scatter <- ggplot(data = mutation_rates, aes(x=positive_mut_rates,y=negative_mut_rates)) +
  ...
  geom_text_repel(data = subset(mutation_rates, gene %in% c("FBXW7","PIK3CA","MAPK1")),aes(x=positive_mut_rates,y=negative_mut_rates,label=gene),size=common.text.size*(5/14),col="black",box.padding = 0.6) +
  labs(y=bquote("Mutation rate in "~HPV^{"−"}~ "\n tumors"), x=bquote("Mutation rate in "~HPV^{"+"}~ "tumors")) + 
  # coord_equal() + 
  # theme_classic() + 
...
gamma_plot <- ggplot(data = gamma.df) + 
  ...
  geom_text_repel(aes(x = gamma_HPVneg, y = gamma_HPVpos,label=Name),
                  box.padding =.23,
                  size=common.text.size*(5/14),segment.alpha=0.6,
                  color="black") + 
...
dendro_and_heatmap_labs <- ggdraw() + 
  draw_plot(heatmap_and_dendro) +
  geom_text(x=.72,y=.95,label="Signatures",size=6) + 
  geom_text(x=.05,y=.42,label="Tumors",size=6,angle=90)
...
```{r combining SNV HPV APOBEC plots, warning=FALSE}
library(cowplot)
