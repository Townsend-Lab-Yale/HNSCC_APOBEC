...
for(i in 1:nrow(mdf)){
 mdf$`HPV status`[i] <- HNSC.selection.HPVknown$HPV_call[which(HNSC.selection.HPVknown$Unique_patient_identifier==as.character(mdf$tumor[i]))[1]]
...
# Heatmap
a <- ifelse(mdf$`HPV status`=="HPV+","red","black")

p1 <- ggplot(mdf, aes(y=variable, x=tumor)) + 
  geom_tile(aes(fill=value)) + scale_fill_gradient2() + theme(axis.text.y = element_text(angle=0)) + theme(axis.text.x=element_text(colour = a,size=2,angle=90,face="bold"))
...
p3 <- ggplot(segment(ddata_y)) + 
  geom_segment(aes(x=x, y=y, xend=xend, yend=yend))  + theme_none
