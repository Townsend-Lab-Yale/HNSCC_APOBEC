...
prev_plot <- ggplot(data = prevalence.df) + 
  ...
  geom_text_repel(data = subset(prevalence.df, (prev_HPVneg>0.025 | prev_HPVpos>0.04) | (prev_HPVneg>0.0 & prev_HPVpos>0)),aes(y = prev_HPVneg, x = prev_HPVpos,label=Name),box.padding =0.6,size=common.text.size*(5/14),color="darkred",fontface="bold") + 
  theme_bw() + 
  ...
  # coord_equal() +  
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +theme(axis.title.x = element_text(margin = margin(t = -.2, r = 0, b = 0, l = 0)))  #+ geom_label(aes(x = prev_HPVneg, y = prev_HPVpos, label=Name))
prev_plot
...
mutation_rates_full_scatter <- ggplot(data = mutation_rates, aes(x=positive_mut_rates,y=negative_mut_rates)) +
  ...
  geom_text_repel(data = subset(mutation_rates, gene %in% c("FBXW7","PIK3CA","MAPK1")),aes(x=positive_mut_rates,y=negative_mut_rates,label=gene),size=common.text.size*(5/14),col="blue",fontface="bold",box.padding = 0.6) +
  labs(y=bquote("Mutation rate in "~HPV^{"-"}~ "\n tumors"), x=bquote("Mutation rate in "~HPV^{"+"}~ "tumors")) + 
  # coord_equal() + 
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_abline(slope=1, intercept=0) + 
  scale_x_continuous(labels=fancy_scientific) + 
  scale_y_continuous(labels=fancy_scientific) + theme(axis.text=element_text(size=common.text.size),
...
combined.plot.prev.and.muts.heat <- plot_grid(prev_plot,mutation_rates_full_scatter,HPV.neg.trinuc.heatmap,HPV.pos.trinuc.heatmap,nrow = 4,align='v',axis='l',rel_heights = c(1,1,.4,.4),labels = "AUTO",label_size = 6)
...
g1 <- ggplot(data=selection.subset.neg.ordered,aes(x=Name,y=mu, fill=Gene)) +
  ...
  theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank()) +
  # geom_text(aes(label=round(mu*1e5,2)), vjust=-0.5, hjust=0.5, position=position_dodge(width=0.9),size=4,angle=90) +
  geom_text(aes(label=freq,y=-1e-7), position=position_dodge(width=0.9),size=common.text.size*(5/14),angle=0,color="black") +
  theme(plot.title = element_text(hjust = 0.5)) +
...
g2 <- ggplot(data=selection.subset.neg.ordered, aes(x=Name,y=gamma_epistasis,fill=Gene)) +
  ...
  coord_flip() + theme(legend.position = c(0.85, .5),legend.text = element_text(size=common.text.size)) + theme(legend.position="none") + scale_y_continuous(labels=fancy_scientific) + theme(axis.text.x = element_text(size =common.text.size)) + theme(plot.title = element_text(size=common.text.size))
# g2
...
g1.pos <- ggplot(data=selection.subset.pos.ordered,aes(x=Name,y=mu, fill=Gene)) +
  ...
  theme(panel.grid.major.y = element_blank(),panel.grid.minor.y = element_blank()) +
  # geom_text(aes(label=round(mu*1e5,2)), vjust=-0.5, hjust=0.5, position=position_dodge(width=0.9),size=4,angle=90) +
  geom_text(aes(label=freq,y=-2.5e-7), position=position_dodge(width=0.9),size=common.text.size*(5/14),angle=0,color="black") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(legend.position = 'none',axis.text.x = element_text(size=common.text.size,angle = 45, hjust = 1)) +
  scale_y_reverse(labels=fancy_scientific) + coord_flip() + 
  theme(axis.text.x = element_text(size = common.text.size))+ theme(plot.title = element_text(size=common.text.size))
...
```{r adding gamma gamma plot to gamma tornado plots}
library(ggplot2);library(ggrepel)
gamma_plot <- ggplot(data = gamma.df) + 
  geom_point(aes(x = gamma_HPVneg, y = gamma_HPVpos),alpha=0.5,size=2) + 
  geom_text_repel(aes(x = gamma_HPVneg, y = gamma_HPVpos,label=Name),
                  box.padding =0.8,
                  size=common.text.size*(5/14),
...
Fig2 <- ggdraw() + 
  ...
  geom_rect(data=data.frame(x=0.715,y=0.23),aes(xmin=x,xmax=x+.28,ymin=y,ymax=y+.52),color="black",fill="white") + 
  draw_plot(gamma_plot,  x=0.62, y=0.2,width = .45,height = .45) + 
  draw_plot_label(c("A", "B","C"), c(0, 0.43,.715), c(1, 1,(.23+.52)), size = common.text.size)

Fig2
ggsave(filename = "Figures/Fig2.png",dpi = 600,width = 3.25*2,height =3,plot = Fig2)
...
HPV_status_vs_APOBEC_weight <- ggplot() + 
  geom_point(data = SNV.APOBEC.df.knownAPOBEC,aes(y=HPV_status_value,x=APOBEC_weight),alpha=0.2,size=3) + 
  stat_smooth(data = SNV.APOBEC.df.knownAPOBEC,aes(y=HPV_status_value,x=APOBEC_weight),method="glm", method.args=list(family="binomial"), se=T) + 
  labs(y="Probability HPV positive",x="APOBEC signature weight") + theme_classic() +
  theme(text = element_text(size=common.text.size)) + 
...
ggsave(plot = HPV_status_vs_APOBEC_weight, filename ="Figures/HPV_status_APOBEC_weight.png",width = 3.25,height = 2,dpi = 600)
...
HPV.vs.APOBEC_ms_forcombine <- ggplot(data = subset(SNV.APOBEC.df, !is.na(SNV.APOBEC.df$APOBEC))) + geom_boxplot(aes(y=SNV_count,x=APOBEC),color="dark red") + geom_jitter(aes(y=SNV_count,x=APOBEC),width= 0.2,alpha=0.5) + facet_grid(.~HPV_status) + theme_classic() + scale_y_log10(labels=c(expression(10^02),expression(10^03)),breaks=c(100,1000)) + labs(x="APOBEC signature detected",y="SNV count")  + theme(strip.text.x = element_text(size=common.text.size)) +theme(axis.text=element_text(size=common.text.size), axis.title=element_text(size=common.text.size,face="bold"))
# plot_grid(
SNV_count_comparing_APOBEC_plot_forcombine <- ggplot(data = subset(SNV.APOBEC.df.comparingAPOBEC, !is.na(SNV.APOBEC.df.comparingAPOBEC$APOBEC))) + geom_boxplot(aes(y=SNV_count,x=HPV_status),color="dark red") + geom_jitter(aes(y=SNV_count,x=HPV_status),width= 0.2,alpha=0.5) + facet_grid(.~APOBEC) + theme_classic() + scale_y_log10(labels=c(expression(10^02),expression(10^03)),breaks=c(100,1000)) + labs(x="HPV status",y="SNV count")  + theme(strip.text.x = element_text(size=common.text.size)) +theme(axis.text=element_text(size=common.text.size), axis.title=element_text(size=common.text.size,face="bold"),axis.title.y = element_blank())

combined_HPV_APOBEC_plot <- plot_grid(HPV.vs.APOBEC_ms_forcombine,SNV_count_comparing_APOBEC_plot_forcombine,align='h',ncol = 2,nrow = 1,labels = c("A","B"),label_size = common.text.size)

combined_HPV_APOBEC_plot

ggsave(plot = combined_HPV_APOBEC_plot,filename = "Figures/combined_APOBEC_vs_HPV.png",height = 3,width = 3.25*2)
...
# Previously broke down each mutation by unique trinucleotide signature 
# (different nucleotide changes can result in the same amino acid)
# now need to remerge because we are focused on the phenotype (Amino acid) 

# Need new data frame for this one, with just rows representing unique amino acids. 
ALL.prev.df$short_name <- NA
for(i in 1:nrow(ALL.prev.df)){
  if(endsWith(x = as.character(ALL.prev.df$Mutation[i]),suffix = "NCSNV")){
    ALL.prev.df$short_name[i] <- as.character(ALL.prev.df$Mutation[i]) 
  }else{
    ALL.prev.df$short_name[i] <- paste(strsplit(x = as.character(ALL.prev.df$Mutation[i]), split=" ")[[1]][1:2],collapse = " ")
  }
}


ALL.prev.df.merged <- as.data.frame(matrix(ncol=5,nrow=length(unique(ALL.prev.df$short_name)),data=NA))
colnames(ALL.prev.df.merged) <- c("Short name", "Median APOBEC contribution", "Median NonAPOBEC contribution", "Frequency", "Selection intensity")
ALL.prev.df.merged$`Short name` <- unique(ALL.prev.df$short_name)
...
for(i in 1:nrow(ALL.prev.df.merged)){
  ALL.prev.df.merged$`Median APOBEC contribution`[i] <- round(median(signature.breakdown.ALL$`APOBEC sum`[which(signature.breakdown.ALL$short_name==ALL.prev.df.merged$`Short name`[i])],na.rm = T),2)
  ALL.prev.df.merged$`Median NonAPOBEC contribution`[i] <- round(median(signature.breakdown.ALL$`NonAPOBEC sum`[which(signature.breakdown.ALL$short_name==ALL.prev.df.merged$`Short name`[i])],na.rm = T),2)
  ALL.prev.df.merged$Frequency[i] <- length(which(signature.breakdown.ALL$short_name==ALL.prev.df.merged$`Short name`[i]))
  ALL.prev.df.merged$`Selection intensity`[i] <- ALL.prev.df$`Selection intensity`[which(ALL.prev.df$short_name==ALL.prev.df.merged$`Short name`[i])[1]]
}
...
if(length(which(is.na(ALL.prev.df.merged$`Median APOBEC contribution`)))>0){
  message(paste(length(which(is.na(ALL.prev.df.merged$`Median APOBEC contribution`)))," substitutions occur in only tumors
that did not have enough mutations to measure trinucleotide context
and will be removed from the analysis",sep=""))
  ALL.prev.df.merged <- ALL.prev.df.merged[-which(is.na(ALL.prev.df.merged$`Median APOBEC contribution`)),]
}

ALL.prev.original <- ALL.prev.df

ALL.prev.df <- ALL.prev.df.merged
...
```{r Selection intensity vs TCW to TKW}
...
distribution.of.APOBEC.TCW <- ggplot(data = ALL.prev.df) + 
  ...
  geom_jitter(aes(y=`Selection intensity`,x=`APOBEC type TCW`),alpha=0.4,width  = 0.1,size=1.5) + 
  theme_classic() + 
  labs(y="Selection intensity") + 
  scale_y_log10(labels=c(expression(10^03),expression(10^05)),breaks=c(1000,100000)) + 
  theme(axis.text=element_text(size=common.text.size), axis.title=element_text(size=common.text.size,face="bold")) + 
  labs(x=expression(TCW %->% TKW))

ggsave(filename = "Figures/selection_vs_TCW.png",plot = distribution.of.APOBEC.TCW,width = 3.25,height = 3.25,dpi = 600)
...
ALL.prev.df.split$Name[which(ALL.prev.df.split$Name=="CCDC50 A 191098615 C NCSNV")] <- "CCDC50 Splice Site"
...
selection.from.contexts.labels <- ggplot(data = ALL.prev.df.split) +#geom_violin(aes(x=`APOBEC context`, y = `Selection from mutational context`))  +
  ...
  geom_text_repel(data = subset(ALL.prev.df.split,`APOBEC context`=="APOBEC processes" & `Selection from mutational context` > 100),aes(x=`APOBEC context`, y = `Selection from mutational context`,label=Name),color="darkred",fontface="bold",size=common.text.size*(5/14)) + #scale_y_log10(labels=fancy_scientific) + 
  geom_text_repel(data = subset(ALL.prev.df.split,`APOBEC context`=="non-APOBEC processes" & `Selection from mutational context` > 650),aes(x=`APOBEC context`, y = `Selection from mutational context`,label=Name),box.padding = .31,color="darkred",fontface="bold",size=common.text.size*(5/14)) + 
  labs(y="Net realized selection intensity") + theme_classic() + theme(axis.text=element_text(size=common.text.size), axis.title=element_text(size=common.text.size,face="bold"))
